...
def button(page, modules):
    ...
    for pairs in pairs[page]:
        buttons.append(
            [
                custom.Button.inline(f"{mahakaal_emoji} " + pair, data=f"Information[{page}]({pair})")
                for pair in pairs
            ]
        )
    ...
    buttons.append(
        [
            custom.Button.inline(
               f"◀️ ᏰᎯᏣᏦ {mahakaal_emoji}", data=f"page({(max_pages - 1) if page == 0 else (page - 1)})"
            ),
            custom.Button.inline(
               f"•{mahakaal_emoji} ❌ {mahakaal_emoji}•", data="close"
            ),
            custom.Button.inline(
               f"{mahakaal_emoji} ᏁᏋﾒᎿ ▶️", data=f"page({0 if page == (max_pages - 1) else page + 1})"
            ),
        ]
    )
    return [max_pages, buttons]
    # Changing this line may give error in bot as i added some special cmds in MahakaalBot channel to get this module work...
...
if Var.TG_BOT_USER_NAME_BF_HER is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        result = None
        query = event.text
        if event.query.user_id == bot.uid and query == "@Official_MahakaalBot_Support":
            rev_text = query[::-1]
            ...
            result = builder.article(
                ...
                buttons=[
                    ...
                    [
                        ...
                        custom.Button.url
                    (
                            "😁 TUTORIAL 😁", "https://youtu.be/Jl32-SOw15o"
                    )
                    ],
                ],
                link_preview=True,
    ...
    async def page(event):
        if not event.query.user_id == bot.uid:
            return await event.answer(
                "HELLO THERE. PLEASE MAKE YOUR OWN MAHAKAALBOT AND USE. © MahakaalBot ™",
                cache_time=0,
                alert=True,
            )
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:
            ...
            await event.answer(mahakaal_alert, cache_time=0, alert=True)
