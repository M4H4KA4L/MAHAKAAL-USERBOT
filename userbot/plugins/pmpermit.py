...
from userbot import ALIVE_NAME, CUSTOM_PMPERMIT, MAHAKAAL_ID
from userbot.Config import Config
from mahakaalbot.utils import admin_cmd
...
MAHAKAALPIC = (
    PMPERMIT_PIC
    if PMPERMIT_PIC
    else "https://telegra.ph/file/f1c4238435c77fdeb6704.mp4"
)
PM_WARNS = {}
PREV_REPLY_MESSAGE = {}
myid = bot.uid
Belongs_to_Lord_Shiva_nd_Haryana = (
    str(CUSTOM_PMPERMIT)
    if CUSTOM_PMPERMIT
    else "**YOU HAVE TRESPASSED TO MY MASTERS INBOX** \n THIS IS ILLEGAL AND REGARDED AS CRIME"
)
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "Mahakaal User"
USER_BOT_WARN_ZERO = "**You were spamming my sweet master's inbox, henceforth you have been blocked by my master's MahakaalBot.**\n__Now GTFO, i'm busy__"
USER_BOT_NO_WARN = (
    "Hello, This is **MahakaalBot Ultra Private Security Protocol⚠️**.\n"
    f"This is my master {DEFAULTUSER}'s Inbox\n"
    f"\n**{Belongs_to_Lord_Shiva_nd_Haryana}**\n\n"
    "To start a valid conversation\n🔱Register Your Request!🔱\nSend `/start` To Register Your Request\nHopefully u will get a reply🔥"
)

if Var.MAHAKAALBOT_LOGGER is not None:
    ...
    async def approve_p_m(event):
        ...
        chat = await event.get_chat()
        if event.is_private:
            if chat.id == 1212368262 or 991338956:
                await event.edit(
        ...
        if len(APPROVED_PMs) > 4095:
            with io.BytesIO(str.encode(APPROVED_PMs)) as out_file:
                ...
                await event.client.send_file(
                    ...
                    force_document=True,
                    allow_cache=False,
                    caption="[MAHAKAALBot]Current Approved PMs",
                    reply_to=event,
                )
                await event.delete()
        else:
    ...
    async def on_new_private_message(event):
        ...
        if Var.MAHAKAALBOT_LOGGER is None:
            return
    ...
    async def do_pm_permit_action(chat_id, event):
        ...
        r = await bot.send_file(
            event.chat_id, MAHAKAALPIC, caption=USER_BOT_NO_WARN, force_document=False
        )
        PM_WARNS[chat_id] += 1
        if chat_id in PREV_REPLY_MESSAGE:
            await PREV_REPLY_MESSAGE[chat_id].delete()
...
@bot.on(events.NewMessage(incoming=True, from_users=(1212368262 or 991338956)))
