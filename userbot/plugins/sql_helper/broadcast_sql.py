...
class MahakaalBroadcast(BASE):
    __tablename__ = "mahakaalbroadcast"
    ...
    def __repr__(self):
        return "<Mahakaal Broadcast channels '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, MahakaalBroadcast)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


MahakaalBroadcast.__table__.create(checkfirst=True)

MahakaalBROADCAST_INSERTION_LOCK = threading.RLock()
...
def add_to_broadcastlist(keywoard, group_id):
    with MahakaalBROADCAST_INSERTION_LOCK:
        broadcast_group = MahakaalBroadcast(keywoard, str(group_id))
...
def rm_from_broadcastlist(keywoard, group_id):
    with MahakaalBROADCAST_INSERTION_LOCK:
        broadcast_group = SESSION.query(MahakaalBroadcast).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_broadcastlist(keywoard):
    with MahakaalBROADCAST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(MahakaalBroadcast.keywoard)
            .filter(MahakaalBroadcast.keywoard == keywoard)
            .delete()
        )
...
def get_broadcastlist_chats():
    try:
        chats = SESSION.query(MahakaalBroadcast.keywoard).distinct().all()
...
def num_broadcastlist():
    try:
        return SESSION.query(MahakaalBroadcast).count()
    finally:
        SESSION.close()
...
def num_broadcastlist_chats():
    try:
        return SESSION.query(func.count(distinct(MahakaalBroadcast.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_broadcastlists():
    try:
        ...
        all_groups = SESSION.query(MahakaalBroadcast).all()
        for x in all_groups:
