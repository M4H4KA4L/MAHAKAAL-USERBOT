...
from mahakaalbot.utils import admin_cmd, register
...
MAHAKAALBOT_LOGGER = True
MAHAKAALBOT_LOGGER = Config.MAHAKAALBOT_LOGGER
...
async def log(log_text):
    ...
    if MAHAKAALBOT_LOGGER:
        if log_text.reply_to_msg_id:
            ...
            await reply_msg.forward_to(MAHAKAALBOT_LOGGER)
        elif log_text.pattern_match.group(1):
            user = f"#LOG / Chat ID: {log_text.chat_id}\n\n"
            textx = user + log_text.pattern_match.group(1)
            await bot.send_message(MAHAKAALBOT_LOGGER, textx)
        else:
...
async def monito_p_m_s(event):
    ...
    if Config.NC_LOG_P_M_S and not sender.bot:
        ...
        if chat.id not in NO_PM_LOG_USERS and chat.id != bot.uid:
            try:
                e = await bot.get_entity(int(Config.MAHAKAALBOT_LOGGER))
...
async def set_no_log_p_m(event):
    if Config.MAHAKAALBOT_LOGGER is not None:
