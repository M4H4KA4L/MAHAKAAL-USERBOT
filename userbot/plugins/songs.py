...
from mahakaalbot.utils import admin_cmd, sudo_cmd, edit_or_reply, progress
...
async def nope(Belongs_to_Lord_Shiva_nd_Haryana):
    ...
    troll = await bot.inline_query("iLyricsBot", f"{(deEmojify(mahakaal))}")
    ...
    async with borg.conversation("@DeezLoadBot") as conv:
        try:
            ...
            await borg.send_file(
                event.chat_id,
                songh,
                caption="🔆**Here's the requested song!**🔆\n`Check out` [MahakaalBot](https://t.me/Official_MahakaalBot_Chat)",
            )
            await event.delete()
        except YouBlockedUserError:
