...
from mahakaalbot.utils import admin_cmd, edit_or_reply, progress, sudo_cmd
...
async def _(mahakaal):
    if mahakaal.fwd_from:
        return
    reply_to_id = mahakaal.message.id
    if mahakaal.reply_to_msg_id:
        reply_to_id = mahakaal.reply_to_msg_id
    event = await edit_or_reply(mahakaal, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await mahakaal.client.download_media(
            reply_message, downloaded_file_name
        )
        if os.path.exists(downloaded_file_name):
            caat = await mahakaal.client.send_file(
                event.chat_id,
...
async def _(event):
    if event.fwd_from:
        return
    mahakaalreply = await event.get_reply_message()
    if not mahakaalreply or not mahakaalreply.media or not mahakaalreply.media.document:
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    if mahakaalreply.media.document.mime_type != "application/x-tgsticker":
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    reply_to_id = event.message
    if event.reply_to_msg_id:
        reply_to_id = await event.get_reply_message()
    chat = "@tgstogifbot"
    mahakaalevent = await edit_or_reply(event, "`Converting to gif ...`")
    async with event.client.conversation(chat) as conv:
        try:
            await silently_send_message(conv, "/start")
            await event.client.send_file(chat, mahakaalreply.media)
            response = await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            if response.text.startswith("Send me an animated sticker!"):
                return await mahakaalevent.edit("`This file is not supported`")
            mahakaalresponse = response if response.media else await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            mahakaalfile = Path(await event.client.download_media(mahakaalresponse, "./temp/"))
            mahakaalgif = Path(await unzip(mahakaalfile))
            ...
            for files in (mahakaalgif, mahakaalfile):
                if files and os.path.exists(files):
                    os.remove(files)
        except YouBlockedUserError:
            await mahakaalevent.edit("Unblock @tgstogifbot")
            return
