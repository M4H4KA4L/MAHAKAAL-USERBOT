...
mahakaalbot = Config.CUSTOM_STICKER_PACK_NAME
...
async def kang(args):
    ...
    if photo:
        ...
        packname = f"HB_{user.username}_MahakaalBot_{pack}"
        packnick = (
            f"{mahakaalbot} Vol.{pack}"
            if mahakaalbot
            else f"@{user.username}'s MahakaalBot Vol.{pack}"
        )
        cmd = "/newpack"
        file = io.BytesIO()
