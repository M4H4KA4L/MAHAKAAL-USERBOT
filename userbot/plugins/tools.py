...
async def _(event):
    ...
    if reply_message.sender.bot:
        ...
        return
    mahakaalevent = await edit_or_reply(event, " `Scanning This media..... wait👀`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await mahakaalevent.edit("`Please unblock `@DrWebBot `and try again`")
            return
...
async def _(event):
    if event.fwd_from:
        return
    mahakaalevent = await edit_or_reply(event, "...")
    start = datetime.now()
    ...
    try:
        ...
        await mahakaalevent.edit(str(e))
        return
    end = datetime.now()
    ms = (end - start).seconds
    await mahakaalevent.edit("Created BarCode in {} seconds🤓".format(ms))
...
async def _(event):
    if event.fwd_from:
        return
    mahakaalevent = await edit_or_reply(event, "`processiong...........`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
...
Year: {}""".format(
        ...
        await mahakaalevent.edit(output_str, link_preview=True)
    else:
        await mahakaalevent.edit("xkcd n.{} not found!".format(xkcd_id))
...
CmdHelp("tools").add_command(
  ...
  "currency", "<amount> <from> <to>", "Currency converter for MahakaalBot", ".currency 10 usd inr"
).add_command(
