...
async def download(event):
    ...
    if Var.GIT_REPO_NAME is None:
        await edit_or_reply(event, "`Please ADD Proper Github Repo Name of mahakaalBot`")
        return
    mahakaalbot = await edit_or_reply(event, "Processing ...")
    ...
    try:
        ...
        await mahakaalbot.edit(str(e))
    else:
        end = datetime.now()
        ms = (end - start).seconds
        await event.delete()
        await mahakaalbot.edit(
            "Downloaded to `{}` in {} seconds.".format(downloaded_file_name, ms)
        )
        await mahakaalbot.edit("Committing to Github....")
        await git_commit(downloaded_file_name, mahakaalbot)


async def git_commit(file_name, mahakaalbot):
    content_list = []
    ...
    for i in content_list:
        ...
        if i == 'ContentFile(path="' + file_name + '")':
            return await mahakaalbot.edit("`File Already Exists`")
            create_file = False
    ...
    if create_file == True:
        ...
        try:
            ...
            await mahakaalbot.edit("Cannot Upload Plugin")
    else:
        return await mahakaalbot.edit("`Committed Suicide`")
