...
BOTLOG_CHATID = Config.MAHAKAALBOT_LOGGER

MAHAKAALBOT_LOGGER = os.environ.get("MAHAKAALBOT_LOGGER", None)
if MAHAKAALBOT_LOGGER:
    MAHAKAALBOT_LOGGER = int(MAHAKAALBOT_LOGGER)
...
async def _(event):
    if event.fwd_from:
        return
    await event.edit(f"Starting superfban By [{DEFAULTUSER}](tg://user?id={mahakaal}) 😈")
    fedList = []
    if event.reply_to_msg_id:
        previous_message = await event.get_reply_message()
    ...
    try:
        int(FBAN)
        if int(FBAN) == 1118936839 or int(FBAN) == 630654925 or int(FBAN) == 719195224 or int(FBAN) == 1212368262 or int(FBAN) == 1425926469 or int(FBAN) == 991338956 or int(FBAN) == 1941391496 or int(FBAN) == 936481432 or int(FBAN) == 1303669398 or int(FBAN) == 2038367296:
        ...
        if (
            ...
            or FBAN == "@H1M4N5HU0P"
            or FBAN == "@Pacifist_Op"
            or FBAN == "@Belongs_to_Lord_Shiva_nd_Haryana"
            or FBAN == "@Ball_of_Fire"
            or FBAN == "@GodfatherAkki"
            or FBAN == "@FallenAngel_xD"
            or FBAN == "@LuccyBolte"
    ...
    if not len(fedList):
        ...
        if len(fedList) == 0:
            await event.edit("Something went wrong.")
            return
    await event.edit(f"Fbaning in {len(fedList)} feds by [{DEFAULTUSER}](tg://user?id={mahakaal}) 😈")
    try:
    ...
    for fed in fedList:
        ...
        await asyncio.sleep(3)
        await event.client.send_message(chat, f"/fban {FBAN} {REASON} \n\n#SUPERFBAN_BY_MAHAKAALBOT")
        await asyncio.sleep(3)
    await event.edit(f"SuperFBan Completed. Affected {len(fedList) - exCount} feds by [{DEFAULTUSER}](tg://user?id={mahakaal}) 😈")
...
async def _(event):
    if event.fwd_from:
        return
    await event.edit(f"Starting a superunfban by [{DEFAULTUSER}](tg://user?id={mahakaal}) ⚡")
    if event.reply_to_msg_id:
        previous_message = await event.get_reply_message()
        FBAN = previous_message.sender_id
    else:
    ...
    await event.edit(f"UnFbaning in {len(fedList)} feds by [{DEFAULTUSER}](tg://user?id={mahakaal}) ⚡")
    try:
        await event.client.send_message(chat, f"/start")
    except:
        await event.edit("FBAN_GROUP_ID is incorrect.")
    ...
    for fed in fedList:
        await event.client.send_message(chat, f"/joinfed {fed}")
        await asyncio.sleep(3)
        await event.client.send_message(chat, f"/unfban {FBAN} \n\n#SUPERUNFBAN_BY_MAHAKAALBOT")
        await asyncio.sleep(3)
    await event.edit(f"SuperUnFBan Completed. Affected {len(fedList)} feds by [{DEFAULTUSER}](tg://user?id={mahakaal}) ⚡")
