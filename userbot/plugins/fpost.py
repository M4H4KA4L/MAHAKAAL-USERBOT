...
async def all_groups_id(mahakaal):
    mahakaalgroups = []
...
async def _(event):
    ...
    if Config.MAHAKAALBOT_LOGGER is None:
        await edit_or_reply(
            event,
            "Please set the required environment variable `MAHAKAALBOT_LOGGER` for this plugin to work",
        )
        return
    try:
        e = await event.client.get_entity(Config.MAHAKAALBOT_LOGGER)
