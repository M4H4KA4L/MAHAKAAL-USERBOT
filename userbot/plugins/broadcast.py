...
async def mahakaalbroadcast_send(event):
    if event.fwd_from:
        return
    mahakaalinput_str = event.pattern_match.group(1)
    if not mahakaalinput_str:
    ...
    keyword = mahakaalinput_str.lower()
    ...
    mahakaalevent = await edit_or_reply(
        event,
    ...
    await mahakaalevent.edit(resultext)
    if BOTLOG:
...
async def mahakaalbroadcast_add(event):
    if event.fwd_from:
        return
    mahakaalinput_str = event.pattern_match.group(1)
    if not mahakaalinput_str:
...
async def mahakaalbroadcast_remove(event):
    if event.fwd_from:
        return
    mahakaalinput_str = event.pattern_match.group(1)
    if not mahakaalinput_str:
...
async def mahakaalbroadcast_list(event):
    if event.fwd_from:
        return
    mahakaalinput_str = event.pattern_match.group(1)
    if not mahakaalinput_str:
    ...
    await edit_or_reply(mahakaalevent, finaloutput)
...
async def mahakaalbroadcast_delete(event):
    if event.fwd_from:
        return
    mahakaalinput_str = event.pattern_match.group(1)
    check1 = sql.num_broadcastlist_chat(mahakaalinput_str)
    if check1 < 1:
        return await edit_delete(
            event,
            f"Are you sure that there is category {mahakaalinput_str}",
            parse_mode=parse_pre,
        )
    try:
        sql.del_keyword_broadcastlist(mahakaalinput_str)
        await edit_or_reply(
            event,
            f"Successfully deleted the category {mahakaalinput_str}",
            parse_mode=parse_pre,
        )
    except Exception as e:
        await edit_delete(
