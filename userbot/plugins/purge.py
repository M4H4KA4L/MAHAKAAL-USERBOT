...
from mahakaalbot.utils import admin_cmd, errors_handler, sudo_cmd, edit_or_reply
...
async def fastpurger(purg):
    ...
    if MAHAKAALBOT_LOGGER:
        await purg.client.send_message(
            MAHAKAALBOT_LOGGER, "Purge of " + str(count) + " messages done successfully."
        )
    await sleep(2)
    await done.delete()
...
async def selfdestruct(destroy):
    ...
    await smsg.delete()
    if MAHAKAALBOT_LOGGER:
        await destroy.client.send_message(MAHAKAALBOT_LOGGER, "sd query done successfully")