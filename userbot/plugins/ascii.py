...
async def _(event):
    ...
    if reply_message.sender.bot:
        ...
        return
    Belongs_to_Lord_Shiva_nd_Haryana = await edit_or_reply(event, "Wait making ASCII...🤓🔥🔥")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await Belongs_to_Lord_Shiva_nd_Haryana.edit("`Please unblock @asciiart_bot and try again`")
            return
        if response.text.startswith("Forward"):
            await Belongs_to_Lord_Shiva_nd_Haryana.edit(
...
async def _(event):
    ...
    if reply_message.sender.bot:
        await edit_or_reply(event, "Reply to actual users message.😒🤐")
        return
    Belongs_to_Lord_Shiva_nd_Haryana = await edit_or_reply(event, "`Processing`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await Belongs_to_Lord_Shiva_nd_Haryana.edit("Please unblock @Lines50Bot and try again")
            return
        await Belongs_to_Lord_Shiva_nd_Haryana.delete()
        await event.client.send_file(
            event.chat_id,
