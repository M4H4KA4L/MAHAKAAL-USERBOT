...
async def _(mahakaalevent):
    event = await edit_or_reply(mahakaalevent, "`Processing.....`")
    fnt = await get_font_file(mahakaalevent.client, "@D3VIL_FONTSS")
    if mahakaalevent.reply_to_msg_id:
        rply = await mahakaalevent.get_reply_message()
    ...
    text = mahakaalevent.pattern_match.group(1)
    if len(text) <= 8:
        font_size_ = 150
        strik = 10
    elif len(text) >= 9:
    ...
    await bot.send_file(
        mahakaalevent.chat_id,
        file_name,
        caption=f"**мα∂ε вү  :** 『[{DEFAULTUSER}](tg://user?id={mahakaal})』\n\n",
    )
    await event.delete()
    try:
        os.remove(file_name)
