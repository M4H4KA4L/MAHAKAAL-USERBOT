...
"""
# Kang with credits. Using in mahakaalbot...
...
async def mahakaalbot(mahakaalmemes):
    ...
    if not replied:
        await edit_or_reply(
            mahakaalmemes, "`Media file not supported. Reply to a supported media`"
        )
        return
    if replied.media:
        mahakaalmemmes = await edit_or_reply(mahakaalmemes, "`Detecting Threats.........`")
    else:
        await edit_or_reply(
            mahakaalmemes, "`Media file not supported. Reply to a suported media`"
        )
        return
    try:
        ...
        pass
    download_location = await mahakaalmemes.client.download_media(replied, "./temp/")
    if download_location.endswith((".webp")):
        download_location = convert_toimage(download_location)
    size = os.stat(download_location).st_size
    if download_location.endswith((".jpg", ".jpeg", ".png", ".bmp", ".ico")):
        if size > 5242880:
            await mahakaalmemmes.edit(
                "`The replied file is not supported. It should be less than 5mb -_-`"
            )
            os.remove(download_location)
            return
        await mahakaalmemmes.edit("`Detected Threats....`")
    else:
        await mahakaalmemmes.edit("`the replied file is not supported`")
        os.remove(download_location)
        return
    try:
        ...
        await mahakaalmemmes.edit("ERROR: " + str(exc))
        os.remove(download_location)
        return
    mahakaal = f"https://telegra.ph{response[0]}"
    mahakaal = await threats(mahakaal)
    await mahakaalmemmes.delete()
...
async def mahakaalbot(mahakaalmemes):
    ...
    if not replied:
        ...
        return
    if replied.media:
        mahakaalmemmes = await edit_or_reply(mahakaalmemes, "`Detecting Trash....`")
    else:
        await edit_or_reply(
            mahakaalmemes, "`Media file not supported. Reply to a suported media`"
        )
    ...
    if download_location.endswith((".jpg", ".jpeg", ".png", ".bmp", ".ico")):
        if size > 5242880:
            ...
            os.remove(download_location)
            return
        await mahakaalmemmes.edit("`Detected Trash.....`")
    else:
        await mahakaalmemmes.edit("Media file not supported. Reply to a suported media")
        os.remove(download_location)
        return
    try:
        response = upload_file(download_location)
...
async def mahakaalbot(mahakaalmemes):
    input_str = mahakaalmemes.pattern_match.group(1)
    ...
    if not replied:
        ...
        return
    if replied.media:
        mahakaalmemmes = await edit_or_reply(mahakaalmemes, "`Trapping.....`")
    else:
        await edit_or_reply(
            mahakaalmemes, "Media file not supported. Reply to a suported media"
        )
    ...
    if download_location.endswith((".jpg", ".jpeg", ".png", ".bmp", ".ico")):
        if size > 5242880:
            ...
            os.remove(download_location)
            return
        await mahakaalmemmes.edit("`Trapped...`")
    else:
        await mahakaalmemmes.edit("Media file not supported. Reply to a suported media")
        os.remove(download_location)
        return
    try:
        ...
        return
    mahakaal = f"https://telegra.ph{response[0]}"
    mahakaal = await trap(text1, text2, mahakaal)
    await mahakaalmemmes.delete()
    await mahakaalmemes.client.send_file(mahakaalmemes.chat_id, mahakaal, reply_to=replied)
...
async def mahakaalbot(mahakaalmemes):
    ...
    if not replied:
        ...
        return
    if replied.media:
        mahakaalmemmes = await edit_or_reply(mahakaalmemes, "`Making A Comment`.")
    else:
        await edit_or_reply(
            mahakaalmemes, "Media file not supported. Reply to a suported media"
        )
    ...
    if download_location.endswith((".jpg", ".jpeg", ".png", ".bmp", ".ico")):
        if size > 5242880:
            ...
            os.remove(download_location)
            return
        await mahakaalmemmes.edit("Commented....")
    else:
        await mahakaalmemmes.edit("Media file not supported. Reply to a suported media")
        os.remove(download_location)
        return
    try:
        ...
        return
    mahakaal = f"https://telegra.ph{response[0]}"
    mahakaal = await phcomment(mahakaal, text, username)
    await mahakaalmemmes.delete()
    await mahakaalmemes.client.send_file(mahakaalmemes.chat_id, mahakaal, reply_to=replied)


CmdHelp("prank").add_command(
  "phc", "<reply to img> <name> - <comment>", "Changes the given pic to dp and shows a comment in phub with the given name", "<reply to img/stcr> .phc NAME - mahakaalO PHUB"
).add_command(
  ...
  "prank", None, "If this module doesn't work then contact admins in @Official_MahakaalBot_Chat"
).add()