...
async def get_users(event):
    ...
    if not sender.id == me.id:
        mahakaal = await edit_or_reply(event, "`processing...`")
    else:
        mahakaal = await edit_or_reply(event, "`processing...`")
    Belongs_to_Lord_Shiva_nd_Haryana = await get_chatinfo(event)
    chat = await event.get_chat()
    if event.is_private:
        return await mahakaal.edit("`Sorry, Cant add users here`")
    s = 0
    f = 0
    error = "None"

    await mahakaal.edit("**TerminalStatus**\n\n`Collecting Users.......`")
    async for user in event.client.iter_participants(Belongs_to_Lord_Shiva_nd_Haryana.full_chat.id):
        try:
