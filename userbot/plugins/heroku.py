...
from userbot.helpers.extras import delete_mahakaal, edit_or_reply as eor
...
async def _(dyno):
    if (HEROKU_APP_NAME is None) or (HEROKU_API_KEY is None):  
        return await eor(dyno, f"Make Sure Your HEROKU_APP_NAME & HEROKU_API_KEY are filled correct. Visit @Official_MahakaalBot_Support for help.", link_preview=False)
    try:
        Heroku = heroku3.from_key(HEROKU_API_KEY)
        app = Heroku.app(HEROKU_APP_NAME)        
    except BaseException:
        return await dyno.reply(f"Make Sure Your Heroku App Name & API Key are filled correct. Visit @Official_MahakaalBot_Support for help.", link_preview=False)
   # event = await eor(dyno, "Downloading Logs...")
    mahakaal_data = app.get_log()
    await eor(dyno, mahakaal_data, deflink=True, linktext=f"**🗒️ Heroku Logs of 💯 lines. 🗒️**\n\n🌟 **Bot Of :** [{MAHAKAAL_NAME}](tg://user?id={Belongs_to_Lord_Shiva_nd_Haryana})\n\n🚀** Pasted**  ")
