...
async def bigspam(mahakaal):
    if not mahakaal.text[0].isalpha() and mahakaal.text[0] not in ("/", "#", "@", "!"):
        mahakaal_msg = mahakaal.text
        mahakaalbot_count = int(mahakaal_msg[9:13])
        mahakaal_spam = str(mahakaal.text[13:])
        for i in range(1, mahakaalbot_count):
            await mahakaal.respond(mahakaal_spam)
        await mahakaal.delete()
        if LOGGER:
            await mahakaal.client.send_message(
...
CmdHelp("spam").add_command(
  ...
  "bigspam", "<count> <text>", "Sends the text 'X' number of times. This what mahakaalbot iz known for. The Best BigSpam Ever", ".bigspam 5000 Hello"
).add()
