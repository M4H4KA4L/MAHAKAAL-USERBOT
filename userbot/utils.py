...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        LOGS.info("MahakaalBot - Successfully imported " + shortname)
    else:
        import userbot.utils
        ...
        mod.delete_mahakaal = delete_mahakaal
        mod.media_type = media_type
        # support for mahakaalbot originals
        sys.modules["mahakaalbot.utils"] = userbot.utils
        sys.modules["mahakaalbot"] = userbot
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.COMMAND_HAND_LER) == 2:
                mahakaalreg = "^" + Config.COMMAND_HAND_LER
                reg = Config.COMMAND_HAND_LER[1]
            elif len(Config.COMMAND_HAND_LER) == 1:
                mahakaalreg = "^\\" + Config.COMMAND_HAND_LER
                reg = Config.COMMAND_HAND_LER
            args["pattern"] = re.compile(mahakaalreg + pattern)
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.SUDO_COMMAND_HAND_LER) == 2:
                mahakaalreg = "^" + Config.SUDO_COMMAND_HAND_LER
...
async def delete_mahakaal(event, text, time=None, parse_mode=None, link_preview=None):
    parse_mode = parse_mode or "md"
    link_preview = link_preview or False
    time = time or 5
    if event.sender_id in Config.SUDO_USERS:
        ...
        mahakaalevent = await event.edit(
            text, link_preview=link_preview, parse_mode=parse_mode
        )
    await asyncio.sleep(time)
    return await mahakaalevent.delete()
...
def errors_handler(func):
    async def wrapper(errors):
        try:
            ...
            link = "[here](https://t.me/Belongs_to_Lord_Shiva_nd_Haryana)"
...
async def unsavegif(event, mahakaal):
    try:
        await event.client(
            functions.messages.SaveGifRequest(
                id=types.InputDocument(
                    id=mahakaal.media.document.id,
                    access_hash=mahakaal.media.document.access_hash,
                    file_reference=mahakaal.media.document.file_reference,
                ),
                unsave=True,
            )
        )
