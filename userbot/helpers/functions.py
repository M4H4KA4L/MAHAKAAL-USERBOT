...
async def admin_groups(mahakaal):
    mahakaalgroups = []
    async for dialog in mahakaal.client.iter_dialogs():
        ...
        if (
            ...
            mahakaalgroups.append(entity.id)
    return mahakaalgroups
...
async def make_gif(event, file):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await event.client.send_read_acknowledge(conv.chat_id)
            mahakaalfile = await event.client.download_media(mahakaalresponse, "./temp")
            return await unzip(mahakaalfile)
...
async def iphonex(text):
    ...
    mahakaal = r.get("message")
    mahakaalurl = url(mahakaal)
    if not mahakaalurl:
        return "check syntax once more"
    with open("temp.png", "wb") as f:
        f.write(requests.get(mahakaal).content)
...
async def waifutxt(text, chat_id, reply_to_id, bot, borg):
    ...
    mahakaal = await sticcers[0].click("me", hide_via=True)
    if mahakaal:
        await bot.send_file(int(chat_id), mahakaal, reply_to=reply_to_id)
